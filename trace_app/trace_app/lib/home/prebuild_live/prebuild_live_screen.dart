...
import 'prebuild_audio_room_screen.dart';
...
class PreBuildLiveScreen extends StatefulWidget {
  ...
  /// When true, the host is auto-matched into a PK battle as soon as the
  /// room goes live. Used when PK is triggered from a voice (audio) room:
  /// the host is routed here first (camera off), then matching starts
  /// automatically instead of requiring a manual tap on this screen.
  final bool autoStartMatching;
...
class PreBuildLiveScreenState extends State<PreBuildLiveScreen>
  ...
  bool _autoMatchTriggered = false;
  ...
  Widget build(BuildContext context) {
    ...
    final cameraDisabled =
        widget.liveStreaming?.getLiveType == LiveStreamingModel.liveAudio;
    ...
    final hostConfig = ZegoUIKitPrebuiltLiveStreamingConfig.host(
      ...
      ..turnOnCameraWhenJoining = !cameraDisabled;

    // Voice-room-originated PK: no camera at all, only the mic toggle stays.
    if (cameraDisabled) {
      hostConfig.bottomMenuBar.hostButtons = [
    ...
    final hostEvents = ZegoUIKitPrebuiltLiveStreamingEvents(
      ...
      onStateUpdated: (state) {
        ...
        if (widget.autoStartMatching &&
            !_autoMatchTriggered &&
            widget.isHost &&
            state != ZegoLiveStreamingState.idle) {
          _autoMatchTriggered = true;
          startMatchingForPk();
        }
      },
    );
  ...
  setupStreamingLiveQuery() async {
    ...
    subscription!.on(LiveQueryEvent.update,
      ...
      */
      ...
      final cameFromAudioRoom =
          newUpdatedLive.getLiveType == LiveStreamingModel.liveAudio;
      final battleIsOver =
          newUpdatedLive.getBattleStatus != LiveStreamingModel.battleAlive;
      if (cameFromAudioRoom && battleIsOver) {
        QuickHelp.goToNavigatorScreen(
          context,
          PrebuildAudioRoomScreen(
            currentUser: widget.currentUser,
            isHost: widget.isHost,
            liveStreaming: newUpdatedLive,
          ),
        );
        return;
      }
  ...
  startMatchingForPk() async {
    ...
    queryBuilder.whereContainedIn(
        LiveStreamingModel.keyLiveType,
        [LiveStreamingModel.liveVideo, LiveStreamingModel.liveAudio]);
